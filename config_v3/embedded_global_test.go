...
func TestEmbeddedGlobal(t *testing.T) {
	...
	gl := global.(*Global)
	assert.True(t, len(gl.Client.Fronted.Providers["akamai"].Masquerades) > 20, "embedded global config does not contain enough akamai masquerades")
	assert.True(t, len(gl.Client.Fronted.Providers["cloudfront"].Masquerades) > 20, "embedded global config does not contain enough cloudfront masquerades")
