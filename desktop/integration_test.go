	...
	case <-onGeo:
		...
		for i := 0; i < 15; i++ {
			...
			for _, op := range borda.FullyReportedOps {
				if op == "report_issue" || op == "sysproxy_off" || op == "sysproxy_off_force" || op == "sysproxy_clear" || op == "probe" || op == "proxy_rank" || op == "proxy_selection_stability" || "replica_upload" || "replica_view" {
