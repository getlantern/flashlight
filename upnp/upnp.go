...
func (ucl *UpnpClient) ForwardPortWithUpnp(port uint16, layer4Proto string) error {
	...
	if len(ips) == 0 {
		return log.Errorf("failed to fetch any routable ip: %v", err)
	}
	...
	if len(successfulClients) != 0 {
		log.Debugf("Portforward with a previously-working RouterClient worked")
	...
	if len(cls) == 0 {
		return log.Errorf("failed to fetch any router clients: %v", err)
	}
	log.Debugf("Got UPNP router clients: %+v", cls)
	...
	if len(successfulClients) != 0 {
		log.Debugf("Portforward with a new RouterClient worked")
...
func fetchAllRoutableLocalIps() ([]string, error) {
	...
	if len(errs) != 0 {
		return allIps, log.Errorf("%+v", errs)
	}
	return allIps, nil
}
...
func portforwardWithAllClients(
	...
	if len(errs) != 0 {
		return successfulClients, failedClients, log.Errorf("%+v", errs)
	}
