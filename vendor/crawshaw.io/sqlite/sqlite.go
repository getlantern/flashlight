...
// XXX <14-10-21, soltzen> The 'cgo android' flag below causes sqlite to use
// memory for temp files.  Android has no globally-writeable temp directory
// (/data/local/tmp is not consistent across all OSes. Samsung does not
// always respect it, for example).
//
// Furthermore, if you ignore this, the app will crash with an
// SQLITE_IOERR_GETTEMPPATH error.
//
// Finally, we can specify a specific directory in the local app directory
// using [temp_store_directory
// pragma](https://www.sqlite.org/pragma.html#pragma_temp_store_directory),
// but that's deprecated and [alternatives are
// inconsistent](https://sqlite-users.sqlite.narkive.com/Wk8sWRdH/usage-of-temporary-files-of-sqlite3-on-android-sqlite-commands-fail).
//
// P.S. Contrary to what the message below says, you don't need to add this
// flag in the Makefile and deal with platform-detection in the Makefile.
...
// #cgo android CFLAGS: -DSQLITE_TEMP_STORE=3
// // !!! UPDATE THE Makefile WITH THESE DEFINES !!!
//
//
