...
func CreateDialer(configDir, name string, s *config.ProxyConfig, uc common.UserConfig) (balancer.Dialer, error) {
	...
	// Check for prefixes and:
	// - Wrap the implementation with a PrefixImpl struct
	// - Change the dialOrigin function accordingly
	if len(s.Prefixes) > 0 {
		typedPrefixes, err := proxyimpl.NewPrefixSliceFromHexStringSlice(s.Prefixes)
		if err != nil {
			return nil, fmt.Errorf(
				"Unable to convert string prefix slice [%v]: %v",
				s.Prefixes, err)
		}
		p.impl = proxyimpl.NewPrefixImpl(p.impl, typedPrefixes)
		p.dialOrigin = multipathPrefixDialOrigin
	}
	return p, nil
}
