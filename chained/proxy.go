...
func newHTTPSProxy(name string, s *ChainedServerInfo, uc common.UserConfig) (*proxy, error) {
	...
	if err != nil {
		wrapped := errors.Wrap(err).With("addr", s.Addr)
		log.Error(wrapped)
		return nil, wrapped
	}
	x509cert := cert.X509()
		...
		return p.reportedDial(p.addr, p.protocol, p.network, func(op *ops.Op) (net.Conn, error) {
			...
			if !conn.ConnectionState().PeerCertificates[0].Equal(x509cert) {
				...
				certMatchErr := errors.New("Server's certificate didn't match expected! Server had\n%v\nbut expected:\n%v",
					received, expected)
				log.Errorf("Got cert error %v", certMatchErr)
				return nil, op.FailIf(certErr)
			}
...
func newOBFS4Proxy(name string, s *ChainedServerInfo, uc common.UserConfig) (*proxy, error) {
	...
	if err != nil {
		...
		log.Errorf("obfs4 error %v", obfs4Err)
		return nil, obfs4Err
	}
...
func newLampshadeProxy(name string, s *ChainedServerInfo, uc common.UserConfig) (*proxy, error) {
	cert, err := keyman.LoadCertificateFromPEMBytes([]byte(s.Cert))
	if err != nil {
		keyErr := errors.New("Key error %v", errors.Wrap(err).With("addr", s.Addr))
		log.Error("Key error %v", keyErr)
		return nil, keyErr
	}
...
func enableKCP(p *proxy, s *ChainedServerInfo) error {
	...
	if err != nil {
		decodeErr := errors.New("Could not decode kcp transport settings?: %v", err)
		log.Errorf("Decoding error %v", decodeErr)
		return decodeErr
	}
	p.kcpConfig = &cfg
