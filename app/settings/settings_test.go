...
func TestRead(t *testing.T) {
	...
	var uid int64
	s := New()
	s.Configure(&ConfigOpts{"1", "1/1/1", "1/1/1", tmpfile.Name()})
...
func TestPersistAndLoad(t *testing.T) {
	...
	yamlFile := "./test.yaml"
	s := New()
	s.Configure(&ConfigOpts{version, revisionDate, buildDate, yamlFile})
	...
	s.SetUserID(1234)
	s2 := New()
	s2.Configure(&ConfigOpts{version, revisionDate, buildDate, yamlFile})
...
func TestLoadLowerCased(t *testing.T) {
	s := New()
	s.Configure(&ConfigOpts{"", "", "", "./lowercased.yaml"})
