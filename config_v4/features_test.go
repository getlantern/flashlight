...
func TestNoDetour(t *testing.T) {
	...
	for _, country := range []string{"hk", "us", "cn", "uz"} {
		for _, os := range []string{"android", "windows", "darwin", "linux"} {
			assert.True(t, gl.FeatureEnabled(FeatureNoDetour, os, common.DefaultAppName, applicationVersion, 1, false, country), fmt.Sprintf("detour is disabled for %s in %s", os, country))
		}
	}
}
	...
	for _, country := range []string{"hk", "ir", "uz"} {
		for _, os := range []string{"android", "windows", "darwin", "linux"} {
			if country != "ir" || os == "android" {
				assert.True(t, gl.FeatureEnabled(FeatureNoShortcut, os, common.DefaultAppName, applicationVersion, 1, false, country), fmt.Sprintf("shortcut is disabled for %s in %s", os, country))
			}
		}
	}
}
